...
2026-06-16: decimal jitter parsing fix for handwriting generation.


## Cache update
- Added a bottom-right expandable cache drawer.
- Added Save current / Clear current / Clear all cache.
- Cache stores question and answer text only; it does not store DeepSeek API Key.
- Main panels now keep consistent height and use scrolling when content is too long.


## 2026-06 clarity fix
- 手写图增强清晰度：更接近干净扫描件。
- 字距略收紧，降低过度扰动。
- 答案从“过短”改为中短：保留必要步骤，但不写答题思路/正式答案/最终答案。


## formula-readable-fixed
- 修复 LaTeX 显示器直接露出源码的问题。
- 兼容 DeepSeek 偶尔输出的 `\beginvmatrix`、`\veci` 等不规范公式写法。
- 新增分数、矩阵、行列式、向量、根号、上下标的前端可读渲染。
- 手写转换前把公式转成更适合抄写的版本，避免 `\frac{}`、`\begin{}` 原样出现在手写预览。
- DeepSeek 提示词改为标准 LaTeX 输出。


## 本版修改
- 取消独立 LaTeX 显示器，改为单一“答案预览”。
- 答案预览中仍会把常见分数、矩阵、根号等公式转成可读样式，但不再拆成左右两栏。
- 手写生成前继续做公式清理，尽量避免 LaTeX 源码进入手写图。


## SukoyaDraw 图示功能
- 右侧功能栏新增：生成图示、插入答案、导出 SVG、导出 PNG、清空图示。
- DeepSeek 只负责生成结构化画图 JSON，不直接画图，避免图乱。
- 前端内置 Rough 风格 SVG 渲染器，支持函数图、几何图、物理受力图、流程图。
- 不删除原有功能：上传、生成答案、多题、手写预览、导出 PDF、缓存库均保留。

## 本版修复
- 图示预览区固定显示在中间，不再隐藏。
- 中间区域改成三块：答案预览 / 图示预览 / 手写预览。
- “生成图示”按钮改成蓝色主按钮，更容易找到。

## 图示空白修复
- 点击“生成图示”会先立即生成本地兜底图，不会再空白。
- 新增“本地测试图”按钮，不需要等 DeepSeek 也能验证图示模块。
- DeepSeek 返回 none 或 JSON 异常时自动保留本地图示。

## 本次修复
- 图示预览改得更稳：空间直线/平面题、函数题、受力题会优先走更合理的本地示意图。
- 生成手写版时，会把当前图示自动附加到最后一页，不再只显示在前端。
- 手写后处理提高清晰度，同时减轻明显像素感。
- 适当降低抖动与噪点，生成速度也比上一版更快。

## 本次前端爆页修复
- 中间区域改成内部滚动，答案/图示/手写预览不再互相覆盖。
- 右侧功能栏改为内部滚动，不再把页面整体撑爆。
- 图示区域限制最大高度，SVG 不再把前端撑开。
- 生成手写答案 / 导出 PDF 时，如果题目明显需要图，会自动生成本地图示并附到最后一页。

## final-quality-fixed
- 修复 DeepSeek 输出中的 LaTeX 残留：begincases/endcases、cdot、neq、leq/geq、times 等会自动转为可读符号。
- 强化 DeepSeek 提示词，禁止输出复杂 LaTeX 命令，优先输出可直接手写的格式。
- 曲线图不再显示旁边的函数源码标签，坐标轴标签靠近箭头边缘。
- 保留小图插入逻辑：图约占 A4 纸 1/8，优先插到正文下方，放不下再分页。

## 本次生成质量修复
- 增加答案后处理清洗：删除“检查/验算/确实/无交点？检查”等 AI 自我校验语气。
- 自动修复 `cdot/neq/begincases` 等残留，避免答案里出现源码味。
- 重新收紧 DeepSeek 提示词：禁止反问、禁止自我纠错、禁止复杂 LaTeX 命令。
- 保留上一版的小图、透明插图、正文下方插图逻辑。

## 本次字体与公式总修复
- 字体列表改成运行时自动刷新，找不到字体时自动从 backend/font_assets 恢复到 ttf_files。
- 生成手写时如果前端字体值失效，后端自动回退到 sukoya.ttf，不再直接报“没有可用字体”。
- 分数统一转为更像书写的分数字符样式，例如 3/2 → ³⁄₂。
- 乘号统一转为 x，避免 cdot、times、·、× 混乱。
- 保留小图、透明插图、正文下方插图逻辑。

## 本次小更新
- 功能栏新增“手写版插入图示”开关。
- 勾选：手写/PDF 自动带图。
- 取消勾选：只生成文字手写版，不自动插图，不影响图示预览和导出 SVG/PNG。

## 本次小更新
- 答案预览区新增“编辑答案”。
- 修改后点击“结束更改”，手写版/PDF 使用最终修改后的答案。
- 如果正在编辑时直接点生成手写版，会自动先保存修改。

## Sukoya 手写体最终优化方案落地
- 启用 Sukoya 优化渲染器：按字独立渲染，模拟同字多形。
- 字间距随机：每个字独立进行 -3px 到 +4px 波动。
- 字号随机：每个字约 96% 到 105% 缩放。
- 基线漂移：每个字轻微上下浮动。
- 行级疲劳：第二页起逐页略小、略紧。
- 局部连笔：常用词附近允许轻微重叠。
- 数字和数学符号有额外轻微变体。
- 禁止项已处理：不再叠加噪点、模糊、纸张纹理、扫描阴影。

## 快速修复
- 修复后端优化渲染器缺少 `import re` 导致的 `name 're' is not defined`。
